...
async function unstake() {
  const stakeCount = await contract.methods.getStakeCount(userAccount).call();
  for (let i = 0; i < stakeCount; i++) {
    const stakeInfo = await contract.methods.getStakeInfo(userAccount, i).call();
    const unlockTime = Number(stakeInfo.startTime) + Number(stakeInfo.tierDays) * 86400;
    const now = Math.floor(Date.now() / 1000);
    if (!stakeInfo.withdrawn && now >= unlockTime) {
      await contract.methods.unstake(i).send({ from: userAccount });
    }
  }
  alert("🔓 Unstaked successfully (if eligible).");
}
